...
OUT=/home/chokwans99/PSD/ssd/tmp/final_exp2_quant_70b
ART=/data2/chokwans99/awq_artifacts/layerskip_llama2_70b/autoawq_tp4
...
mkdir -p "$OUT/ar"
echo "[$(date +%H:%M:%S)] === ar (TP=4 AWQ-quant 70B) ==="
fuser -k ${SSD_DIST_PORT}/tcp 2>/dev/null || true; sleep 2
SSD_PROFILE_MESA=0 CUDA_VISIBLE_DEVICES=0,1,2,3   "$PY" -O bench/bench.py --llama --size 8 --model_path "$TARGET"     --gpus 4 --b 1 --temp 0.6 --output_len 256 --max_model_len 2048 --all --numseqs 50     $QUANT >"$OUT/ar/run.log" 2>&1
echo "AR TP: $(grep -oP 'Total Throughput:\s*\K[\d.]+' $OUT/ar/run.log | head -1)"
sleep 3
...
for EX in 40 47 53; do
    run "mesa_k5_f4_dfo2_exit${EX}"  --draft_path "$DRAFT" --async --spec --k 5 --f 4         --mesa --mesa_exit_layer $EX --mesa_draft_fan_out 2
done

echo ""
echo "=== SUMMARY ==="
printf "%-28s %-10s %-8s %-8s %-8s %-10s %-10s\n" "tag" "TP" "CH" "TS" "Accept" "Draft_ms" "Verify_ms" | tee "$OUT/SUMMARY.txt"
for f in ar baseline_k7_uniform baseline_k7_geo mesa_k5_f4_dfo2_exit40 mesa_k5_f4_dfo2_exit47 mesa_k5_f4_dfo2_exit53; do
    log="$OUT/$f/run.log"
