#!/bin/bash
# Draft AWQ perf comparison on codellama-34B target.
#
# Compares for each spec config:
#   A. target AWQ + draft DENSE   (baseline — current final_exp2_quant results)
#   B. target AWQ + draft AWQ     (this script's runs)
#
# Writes new runs under tmp/final_exp2_quant/draft_awq/{config}/run.log.
...
OUT=/home/chokwans99/PSD/ssd/tmp/final_exp2_quant/draft_awq
TGT_ART=/data2/chokwans99/awq_artifacts/codellama34b_awq_tp4
...
# No AR here — AR has no draft. Compare only spec configs.
...
echo ""
echo "=== SUMMARY (target AWQ + draft AWQ) ==="
printf "%-28s %-10s %-8s %-8s %-8s %-10s %-10s\n" "tag" "TP" "CH" "TS" "Accept" "Draft_ms" "Verify_ms" | tee "$OUT/SUMMARY.txt"
for f in baseline_k7_uniform baseline_k7_geo mesa_k5_f4_dfo2_exit24 mesa_k5_f4_dfo2_exit28 mesa_k5_f4_dfo2_exit32; do
    log="$OUT/$f/run.log"
