#!/usr/bin/env bash
# MESA K1=7 K2=5 PROFILE_MESA=1 with mesa_exit_layer=52 (vs default 56).
# Paired with the exit=56 cell to isolate the effect of exit_layer.

set -euo pipefail

ROOT="/home/chokwans99/PSD/ssd"
OUTDIR="${ROOT}/experiments/paper_baselines/new_exp/mesa_k1_7_k2_5_dfo2_pfo1_exit52_profile1_seed42_20260521"
...
echo "[$(date -Is)] === START MESA K1=7 K2=5 exit=52 PROFILE_MESA=1 (seed=${SEED}) ===" | tee "${OUTDIR}/run.log"
...
"${PY}" -O bench/bench.py \
  ...
  --async --spec --k 12 --f 3 \
  --mesa --mesa_exit_layer 52 --mesa_phase1_k 7 --mesa_phase2_k 5 \
  --mesa_draft_fan_out 2 --mesa_policy b \
  2>&1 | tee -a "${OUTDIR}/run.log"

echo "[$(date -Is)] === END MESA PROFILE=1 exit=52 (seed=${SEED}) ===" | tee -a "${OUTDIR}/run.log"

{
  echo "seed=${SEED} profile=1 exit_layer=52"
