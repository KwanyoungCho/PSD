...
def derive_status(rows, marker_prefix="target_spec_wait_"):
    ...
    """
    # If any row has a step_id, use it directly.
...
STATUS_ORDER = ["hit_k1", "hit_k2", "hit", "miss"]
...
def main():
    ...
    # Draft rows: if no step_id, assign sequentially using hit_cache_respond_<status>.
    d_has_sid = any(r.get("step_id") is not None for r in d_rows)
    if not d_has_sid:
        d_status_of = _assign_sequential_step_ids(d_rows, "hit_cache_respond_")
    else:
        d_status_of = status_of  # reuse target's mapping when sids are real
    print(f"target rows={len(t_rows):,}  draft rows={len(d_rows):,}  steps={len(status_of):,}")
