...
OUT="${ROOT}/experiments/proxy_async_overlap/b_gt1/b_gt1/jit_subset"
...
# jit_subset A/B — B=16, {JIT-all(base), subset} × {k1x1_d5p1, k2x2_d4p1},
# 3-rep interleaved. PROFILE=0. Only difference between arms:
# SSD_DUET_JIT_SUBSET=0/1.
duet_ab() {
  local label="$1" K1="$2" K2="$3" dfo="$4" pfo="$5" sub="$6"
  local k=$((K1 + K2)) f=$((dfo + pfo))
  ( export SSD_FORCE_SPLIT_K1K2=1 SSD_DUET_JIT_SHORT=1 SSD_DUET_JIT_SUBSET="${sub}"
    run_one "${label}" --b 16 --numseqs 16 \
      --k "${k}" --f "${f}" --duet --duet_exit_layer 56 \
      --duet_phase1_k "${K1}" --duet_phase2_k "${K2}" \
      --duet_draft_fan_out "${dfo}" --duet_policy b )
  PORT=$((PORT + 2))
...
duet_prof() {
  local label="$1" sub="$2"
  ( export SSD_FORCE_SPLIT_K1K2=1 SSD_DUET_JIT_SHORT=1 SSD_DUET_JIT_SUBSET="${sub}"
    export SSD_PROFILE_DUET=1 SSD_PROFILE_DIR="${OUT}/${label}"
    run_one "${label}" --b 16 --numseqs 16 --output_len 128 \
      --k 4 --f 5 --duet --duet_exit_layer 56 \
      --duet_phase1_k 2 --duet_phase2_k 2 \
      --duet_draft_fan_out 4 --duet_policy b )
  PORT=$((PORT + 2))
}
duet_prof prof_k2x2_base 0
duet_prof prof_k2x2_sub 1
echo "[$(date -Is)] === JIT_SUBSET PROF DONE ==="
