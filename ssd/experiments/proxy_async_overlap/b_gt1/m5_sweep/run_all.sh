#!/usr/bin/env bash
# M5 — B>1 perf sweep (docs/duet/13 stage M5): B in {1,2,4}, DUET champion
# (E9K24_jit, m4-smoke args) vs SD-best C (k=7 f=6), INTERLEAVED per B on
# GPUs 0-4. ns=20 out=256 seed 42 temp 0.7 for every cell. Ports 12900+.
set -uo pipefail
ROOT="/home/chokwans99/PSD/ssd"
OUT="${ROOT}/experiments/proxy_async_overlap/b_gt1/m5_sweep"
...
# Champion = m4 smoke DUET args (E9K24_jit).
DUET_ARGS=(
...
cleanup() {
  # Kill ONLY our bench + its multiprocessing.spawn children. The unrelated
  # vLLM on GPUs 6-7 is retitled "VLLM::Worker_TP*" and matches neither.
...
echo "[$(date -Is)] GPU regime at sweep start:"
...
for B in 1 2 4; do
  ...
  PORT=$((PORT + 1))
  run_one "c_b${B}" --b "${B}" "${C_ARGS[@]}"
done

echo "[$(date -Is)] GPU regime at sweep end:"
nvidia-smi --query-compute-apps=gpu_uuid,pid,used_memory,process_name --format=csv
echo "[$(date -Is)] === M5 SWEEP DONE ==="
