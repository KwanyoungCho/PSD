#!/usr/bin/env bash
# Profiler-off full Spec-Bench validation under the native 2,048 contract.
set -euo pipefail
...
TAG=${1:?usage: run_full_arm.sh TAG}
ARM=${HERE}/full/${TAG}
DATA=${DATA:-${BASE}/data/specbench_full.jsonl}
...
OUTLEN=${OUTLEN:-1024}; SEED=${SEED:-42}

mkdir -p "${ARM}"
expected=$(wc -l <"${DATA}")
if (( expected != 560 )); then
  echo "expected full Spec-Bench with 560 turns, got ${expected}: ${DATA}" >&2
  exit 2
fi
actual=0
[[ -e ${ARM}/raw.jsonl ]] && actual=$(wc -l <"${ARM}/raw.jsonl")
if (( actual == expected )) && [[ -e ${ARM}/metrics.json ]]; then
  echo "[$(date -Is)] SKIP complete full arm ${TAG}"
  exit 0
fi
if (( actual > expected )); then
  echo "invalid output row count ${actual}/${expected}: ${ARM}/raw.jsonl" >&2
  exit 3
fi
...
echo "[$(date -Is)] full ${TAG}: resume=${actual}/${expected} " \
     "K=${K1}/${K2} exit=${EXIT_LAYER} N/M=${N1}/${M1},${N2}/${M2} " \
     "R=${ROOT_COUNT} RPP=${RPP}" | tee -a "${ARM}/run.log"

set +e
SSD_DIST_PORT=${PORT} timeout --signal=TERM --kill-after=60s 300m \
  "${PY}" -O "${RUNNER}" \
    ...
    --seed "${SEED}" --data "${DATA}" --resume --out "${ARM}/raw.jsonl" \
    >>"${ARM}/run.log" 2>&1
rc=$?
set -e
...
echo "[$(date -Is)] full ${TAG} complete"
