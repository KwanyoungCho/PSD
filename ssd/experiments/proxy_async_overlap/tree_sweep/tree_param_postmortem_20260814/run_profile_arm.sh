#!/usr/bin/env bash
# Diagnostic-only profile/topology collection. Never report TPS from this run.
set -euo pipefail
...
TAG=${1:?usage: run_profile_arm.sh TAG}
ARM=${HERE}/profiles/${TAG}
DATA=${DATA:-${HERE}/forensic_subset.jsonl}
...
OUTLEN=${OUTLEN:-256}; SEED=${SEED:-42}
TRACE=${TRACE:-0}
...
export SSD_PROFILE_DUET_MAX_EVENTS=120000 SSD_PROFILE_DIR=${ARM}/profile
export SSD_DUET_E0_TRACE=0
unset SSD_TREE_NODE_AUDIT SSD_TREE_STAGE1 SSD_TREE_STAGE2
if [[ ${TRACE} == 1 ]]; then
  export SSD_TREE_TOPO_TRACE=${ARM}/topology
  export SSD_TREE_CALIB_TRACE=${ARM}/calib
else
  unset SSD_TREE_TOPO_TRACE SSD_TREE_CALIB_TRACE
fi

echo "[$(date -Is)] profile ${TAG}: K=${K1}/${K2} exit=${EXIT_LAYER} " \
     "N/M=${N1}/${M1},${N2}/${M2} R=${ROOT_COUNT} RPP=${RPP}" | tee "${ARM}/run.log"

set +e
SSD_DIST_PORT=${PORT} timeout --signal=TERM --kill-after=60s 120m \
  "${PY}" -O "${RUNNER}" \
    ...
    --max_model_len 2048 --template raw --warmup 0 \
...
if [[ ${TRACE} == 1 ]]; then
  "${PY}" "${ROOT}/tools/duet_calibration/analyze_tree_outcomes.py" \
    --trace-prefix "${ARM}/topology" --rerank-caps 4,6,8,10,12,14,16 \
    --json-out "${ARM}/tree_outcomes.json" >"${ARM}/tree_outcomes.txt"
  "${PY}" "${ROOT}/tools/duet_calibration/analyze_round_widths.py" \
    --trace-prefix "${ARM}/topology" --json-out "${ARM}/round_widths.json" \
    >"${ARM}/round_widths.txt"
fi
"${PY}" "${ROOT}/tools/duet_calibration/analyze_k_balance.py" \
  --run "${K1},${K2},${ARM}/profile" --skip-steps 0 --min-steps 30 \
  --json-out "${ARM}/overlap.json" >"${ARM}/overlap.txt"
"${PY}" "${BASE}/analysis/question_level_metrics.py" \
  "${ARM}/raw.jsonl" --json >"${ARM}/metrics.json"
echo "[$(date -Is)] profile ${TAG} complete"
