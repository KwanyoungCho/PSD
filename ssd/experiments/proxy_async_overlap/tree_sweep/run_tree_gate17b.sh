#!/usr/bin/env bash
# 헤드라인 재측정: chain champion vs tree(arena+1b) — eslab17 클린
# 인터리브 3-cycle, verdict급 25×384, PROFILE=0.
set -u
ROOT="$HOME/Parallel_SD/ssd"
PY="/data2/chokwans99/conda_envs/ssd/bin/python"
REV="$(cd "$ROOT" && git rev-parse --short HEAD)"
OUT="$HOME/tree_gate_${REV}"
mkdir -p "${OUT}"
cd "${ROOT}"
...
for cyc in 1 2 3; do
  if [ $((cyc % 2)) -eq 1 ]; then
    run_one "c${cyc}_chain" --duet_tree_policy off
    run_one "c${cyc}_tree" --duet_tree_policy level --duet_tree_nv 8 \
...
for f in "${OUT}"/c*_*.log; do
  ...
  echo "${b} tps=${t} p2al=${a} p2hit=${h} p1al=${p1} tokstep=${ts}"
done
