...
run_one () {
  local label="$1" ct="$2" beta="$3" port="$4" rc="${5:-}"
  extra_rc=""
  [ -n "$rc" ] && extra_rc="--duet_tree_root_count $rc"
  SSD_DIST_PORT=$port $PY -O bench/bench.py --llama --size 8 \
    ...
    --duet_tree_c_tensor "$ct" $extra_rc \
    > "$OUT/$label.log" 2>&1
...
run_one integ_bb_b05 3 0.5 13996
run_one integ_r6_b05 3 0.5 13997 6
echo INTEG_DONE
