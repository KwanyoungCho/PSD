...
set -u
ROOT="$HOME/Parallel_SD/ssd"; PY="/data2/chokwans99/conda_envs/ssd/bin/python"
REV="$(cd "$ROOT" && git rev-parse --short HEAD)"
OUT="${ROOT}/experiments/proxy_async_overlap/tree_sweep/rng_iso17_${REV}"
mkdir -p "${OUT}"; cd "${ROOT}"
...
export PATH=$HOME/cuda129/bin:$PATH SSD_DATASET_DIR=/data2/chokwans99/datasets
...
TREE=(--llama --size 8 --model_path /data2/chokwans99/awq_calibrated_autoawq/layerskip_llama2_70b
