#!/usr/bin/env bash
# Priority-1 A/B: CPU fallback versus GPU target topology/mask preparation.
set -euo pipefail
...
OUT=${ROOT}/experiments/proxy_async_overlap/tree_sweep/target_step_gap_analysis_20260812/gpu_topology_ab
...
REPEAT=${REPEAT:-1}
RUN_SET=${RUN_SET:-all}  # all | cpu | gpu
PHASE=${PHASE:-both}     # both | p1 | p2
DETAIL_PROFILE=${DETAIL_PROFILE:-1}
...
export SSD_PROFILE=0 SSD_PROFILE_DUET="${DETAIL_PROFILE}"
export SSD_PROFILE_DUET_DETAIL="${DETAIL_PROFILE}"
...
run_one() {
  local arm=$1 gpu_path=$2 p1=$3 p2=$4 n1=$5 m1=$6 n2=$7 m2=$8 port=$9
  local stem=${arm}_r${REPEAT}_s42_o256
  ...
  check_gpus
  mkdir -p "${profile}"
  echo "[$(date -Is)] START ${arm} SSD_TREE_TOPOLOGY_GPU=${gpu_path}"
  SSD_TREE_TOPOLOGY_GPU="${gpu_path}" \
  ...
  echo "[$(date -Is)] END ${arm}: 7/7"
}

# Matched target widths isolate fixed tree preparation from extra model rows.
port=$((18710 + (REPEAT - 1) * 10))
if [[ "${RUN_SET}" == all || "${RUN_SET}" == cpu ]]; then
  if [[ "${PHASE}" == both || "${PHASE}" == p1 ]]; then
    run_one p1_cpu 0 on  off 8 8 8 8 "${port}"
  fi
  if [[ "${PHASE}" == both || "${PHASE}" == p2 ]]; then
    run_one p2_cpu 0 off on  14 12 4 4 "$((port + 1))"
  fi
fi
if [[ "${RUN_SET}" == all || "${RUN_SET}" == gpu ]]; then
  if [[ "${PHASE}" == both || "${PHASE}" == p1 ]]; then
    run_one p1_gpu 1 on  off 8 8 8 8 "$((port + 2))"
  fi
  if [[ "${PHASE}" == both || "${PHASE}" == p2 ]]; then
    run_one p2_gpu 1 off on  14 12 4 4 "$((port + 3))"
  fi
fi

echo "[$(date -Is)] GPU topology A/B complete: ${OUT}"
