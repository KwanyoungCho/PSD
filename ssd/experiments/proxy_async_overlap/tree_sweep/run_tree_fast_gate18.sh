#!/usr/bin/env bash
# Fast correctness gate for the frozen tree shape.  This is deliberately not
# a parameter sweep: one seed, short output, chain/arena/executor once each.
set -u

ROOT="/home/chokwans99/PSD/ssd"
PY="/home/chokwans99/anaconda3/envs/ssd/bin/python"
OUT="${ROOT}/experiments/proxy_async_overlap/tree_sweep/fast_gate_current"
mkdir -p "${OUT}"
cd "${ROOT}"
...
cleanup_jobs () {
  for pid in $(nvidia-smi --query-compute-apps=pid \
      --format=csv,noheader | sort -u); do
    if [ "$(ps -o user= -p "${pid}" 2>/dev/null)" = "chokwans99" ]; then
...
run_one () {
  ...
  local rc=$?
  echo "EXIT:${rc}" >> "${log}"
  grep -E "Final Decode Throughput|Phase 1 .*Hit Rate|Phase 2 .*Hit Rate|Phase 1 Accepted Len|Phase 2 Accepted Len|Avg Tokens per step|p2exec stats" "${log}" || true
  cleanup_jobs
  sleep 3
  return "${rc}"
}

run_one chain 15101 0 --duet_tree_policy off
run_one arena_r6 15102 0 --duet_tree_policy level --duet_tree_nv 8 \
  --duet_tree_beta 0.5 --duet_tree_root_count 6
run_one exec_r6 15103 1 --duet_tree_policy level --duet_tree_nv 8 \
  --duet_tree_beta 0.5 --duet_tree_root_count 6
