#!/usr/bin/env bash
# KV-promo A/B: champion config, promo ON vs OFF, same seed, ns=20 out=256.
set -euo pipefail
...
run_one() { local label="$1" port="$2"; shift 2; local o="${PHASE_DIR}/${label}"; mkdir -p "$o"
  ...
  SSD_DIST_PORT=$port SSD_FORCE_SPLIT_K1K2=1 SSD_DUET_JIT_SHORT=1 "$@" timeout 900 "${PY}" -O bench/bench.py "${ARGS[@]}" > "$o/run.log" 2>&1 || echo "CRASH ${label}"
  echo "[$(date -Is)] === END ${label}: $(grep 'Final Decode' "$o/run.log"|tail -1) ==="
}
run_one "ab_off" 12850
run_one "ab_on"  12851 env SSD_DUET_KV_PROMO=1
echo "=== SUMMARY ==="
for l in ab_off ab_on; do echo "-- $l --"; grep -E "Final Decode|Tokens per step \(incl|Cache Hits|Phase 1 Accepted|Phase 2 Accepted|draft step time|target time per full" "${PHASE_DIR}/$l/run.log"|sed 's/\[metrics\] //'; done
echo "[$(date -Is)] === AB DONE ==="
