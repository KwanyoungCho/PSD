#!/usr/bin/env bash
# KV-promo smoke — champion config + SSD_DUET_KV_PROMO=1 SSD_DUET_KV_PROMO_PARITY=1, short run.
# Functional check: promo-corrupted KV would crater acceptance/cache-hit.
set -euo pipefail
ROOT="/home/chokwans99/PSD/ssd"
PHASE_DIR="${ROOT}/experiments/proxy_async_overlap/kv_promo"
PY="/home/chokwans99/anaconda3/envs/ssd/bin/python"
cd "${ROOT}"
export CUDA_VISIBLE_DEVICES=0,1,2,3,4
export SSD_DIST_PORT=12810
...
sleep 5
SSD_FORCE_SPLIT_K1K2=1 SSD_DUET_JIT_SHORT=1 SSD_DUET_KV_PROMO=1 SSD_DUET_KV_PROMO_PARITY=1 \
  timeout 600 "${PY}" -O bench/bench.py \
  --llama --size 8 \
  ...
  --gpus 5 --b 1 --temp 0.7 --seed 42 --numseqs 2 \
  --input_len 512 --output_len 32 --all --max_model_len 2048 \
...
grep -E "Final Decode|Avg Cache Hits|Phase 1 Accepted|Tokens per step \(incl" "${outdir}/run.log" | sed 's/\[metrics\] //' || true
