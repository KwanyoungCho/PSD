#!/bin/bash
# Async SSD baseline (no MESA): k=7, f=2 on 70B both-AWQ.
set -u
cd "$(dirname "$0")/../.."
...
export SSD_PROFILE_MESA=1   # profiling labels also work for non-MESA path
...
OUT="$PWD/experiments/async_ssd_70b/results/k7_f2"
mkdir -p "$OUT"
...
if [ -f "$OUT/run.log" ] && grep -q "Total Throughput" "$OUT/run.log"; then
    echo "[skip] already done"
else
    echo "[$(date +%H:%M:%S)] === async SSD k=7 f=2 (no MESA) ==="
    fuser -k ${SSD_DIST_PORT}/tcp 2>/dev/null || true; sleep 2
    SSD_PROFILE_DIR="$OUT" \
        "$PY" -O bench/bench.py $COMMON $QUANT --k 7 --f 2 \
        >"$OUT/run.log" 2>&1
    rc=$?
...
echo "===== metrics ====="
log="$OUT/run.log"
[ -f "$log" ] && grep -E "Total Throughput|Avg Tokens per step|Avg Fraction|Avg Cache|Avg Phase|Avg draft step|Avg target verify|Avg Tokens per step on" "$log" | head -10
