#!/bin/bash
# True split-K1/K2 mode benchmark on 70B both-AWQ.
# Same config as previous split_dfo_sweep_v5 but with SSD_FORCE_SPLIT_K1K2=1
# (option 2 clean re-architecture, no hybrid dispatch coupling).
...
export SSD_FORCE_SPLIT_K1K2=1   # ★ true split-K1/K2 mode
...
OUT="${1:-$PWD/experiments/split_k1k2_70b/results}"
mkdir -p "$OUT"
echo "Output: $OUT"
echo "Mode: true split-K1/K2 (SSD_FORCE_SPLIT_K1K2=1), K1=7, K2=7, pfo=1"
...
run() {
    ...
    local err=$(grep -oP 'Traceback|RuntimeError|FAIL|Aborted' "$subdir/run.log" | head -1)
