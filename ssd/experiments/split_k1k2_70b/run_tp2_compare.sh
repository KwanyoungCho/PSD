#!/bin/bash
# 70B TP=2 — uniform vs non-uniform Phase 1, exit_layer=57.
# Same GPU set used for both runs to ensure fair comparison.
set -u
cd "$(dirname "$0")/../.."
...
# Need 3 GPUs (TP=2 target + 1 draft).
if [ -z "${CUDA_VISIBLE_DEVICES:-}" ]; then
    ...
    [ "$(echo $free_gpus | tr ',' '\n' | wc -l)" -lt 3 ] && { echo "ERROR: need 3 GPUs"; exit 1; }
...
TGT_ART=/data2/chokwans99/awq_artifacts/layerskip_llama2_70b/autoawq_tp2
DRAFT_ART=/data2/chokwans99/awq_artifacts/tinyllama_1b/draft_tp1

COMMON="--llama --size 8 --model_path $TARGET --draft_path $DRAFT --async --spec --gpus 3 --b 1 --temp 0.6 --output_len 64 --max_model_len 2048 --random --numseqs 10"
...
run_case() {
    ...
    local OUT="$PWD/experiments/split_k1k2_70b/tp2_compare/$tag"
    mkdir -p "$OUT"
    echo "[$(date +%H:%M:%S)] === $tag (TP=2, exit=57, p1=$p1list) ==="
...
print(f'{\"config\":<20} {\"TPS\":>7} {\"avg_len\":>8} {\"accept\":>10} {\"P1_hit\":>7} {\"graph_pre\":>11} {\"phase1_med\":>11} {\"phase2_med\":>11}')
for tag in ['uniform','nonuniform']:
    d = f'$PWD/experiments/split_k1k2_70b/tp2_compare/{tag}'
    ...
    p1m = statistics.median([r['ms'] for r in rows if r['label']=='phase1_replay']) if any(r['label']=='phase1_replay' for r in rows) else 0
    p2m = statistics.median([r['ms'] for r in rows if r['label']=='phase2_replay']) if any(r['label']=='phase2_replay' for r in rows) else 0
    paths_t = sorted(glob.glob(f'{d}/mesa_profile_target_rank0_*.json'))
    rows_t = json.load(open(paths_t[-1]))
    gpm = statistics.median([r['ms'] for r in rows_t if r['label']=='graph_pre']) if rows_t else 0
    print(f'{tag:<20} {tp:>7} {al:>8} {ar:>10} {p1h:>7} {gpm:>11.2f} {p1m:>11.3f} {p2m:>11.3f}')"
