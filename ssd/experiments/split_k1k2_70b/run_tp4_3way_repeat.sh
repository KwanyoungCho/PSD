#!/bin/bash
# Re-run uniform / uniform_via_list / nonuniform back-to-back on same GPUs
# to disambiguate variance vs real implementation difference.
set -u
cd "$(dirname "$0")/../.."
...
run_case() {
    ...
    local OUT="$PWD/experiments/split_k1k2_70b/tp4_3way_repeat/$tag"
    mkdir -p "$OUT"
    echo "[$(date +%H:%M:%S)] === $tag (p1=$p1list) ==="
...
print(f'{\"config\":<20} {\"TPS\":>7} {\"avg_len\":>8} {\"accept\":>8} {\"P1_hit\":>7} {\"target_v\":>9} {\"draft_ms\":>10} {\"phase1_md\":>10} {\"phase2_md\":>10}')
for tag in ['uniform', 'uniform_via_list', 'nonuniform']:
    d = f'$PWD/experiments/split_k1k2_70b/tp4_3way_repeat/{tag}'
    ...
    print(f'{tag:<20} {tp:>7} {al:>8} {ar:>8} {p1h:>7} {tv:>9} {dms:>10} {p1m:>10.3f} {p2m:>10.3f}')"
