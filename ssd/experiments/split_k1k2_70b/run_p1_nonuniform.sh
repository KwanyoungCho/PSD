#!/bin/bash
# Non-uniform Phase 1: [5,3,1,1,0,0,0,0,5], sum=15, K1=K2=8, dfo=2, pfo=1, exit=40.
set -u
cd "$(dirname "$0")/../.."
...
OUT="$PWD/experiments/split_k1k2_70b/p1_nonuniform_5311_5_exit57"
mkdir -p "$OUT"
...
echo "[$(date +%H:%M:%S)] === K1=8 K2=8 phase1_list=[5,3,1,1,0,0,0,0,5] sum=15 exit=57 ==="
...
err=$(grep -oP 'Traceback|RuntimeError|FAIL|Aborted|ValueError' "$OUT/run.log" | head -1)
...
echo "===== metrics ====="
log="$OUT/run.log"
grep -E "MESA split-K1K2.*layouts|Total Throughput|Avg Tokens per step|Avg Fraction|Avg Cache|Avg Phase|Avg draft step|Avg target verify|Avg Tokens per step on" "$log" | head -10
