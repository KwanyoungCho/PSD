...
for ax, proc in zip(axes, ["target", "draft"]):
    # Gather all labels across configs for this proc
    labels_set = set()
    per_cfg = {}
    for cfg, _ in CONFIGS:
        df = _load(cfg)
        if df is None:
            per_cfg[cfg] = pd.Series(dtype=float); continue
        sub = df[df["proc"] == proc].set_index("label")["ms_per_step"]
        per_cfg[cfg] = sub
        labels_set.update(sub.index)

    # Consistent label order: biggest-first based on baseline
    ...
    # Total ms-per-step annotation on top
    ...
    ax.set_title(f"{proc} side — per-phase contribution per step")
