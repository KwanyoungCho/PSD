"""Plot MESA per-phase breakdown. Reads mesa_profile_{draft,target_rank0}.json from OUTDIR.
...
for tag in ("draft", "target_rank0"):
    ...
    if not paths:
        print(f"[warn] missing mesa_profile_{tag}_*.json in {OUTDIR}", file=sys.stderr); continue
    path = paths[-1]
...
print("=== MESA phase breakdown (post-warmup) ===")
print(summary)
...
_is_mesa = df["label"].str.startswith("phase").any()
...
plt.tight_layout()
out = f"{OUTDIR}/mesa_breakdown.png"
...
if (df["proc"] == "draft").any():
    ...
    plt.tight_layout()
    out2 = f"{OUTDIR}/mesa_breakdown_over_time.png"
...
summary.to_csv(f"{OUTDIR}/mesa_breakdown_summary.csv")
print(f"-> saved {OUTDIR}/mesa_breakdown_summary.csv")
...
ps.to_csv(f"{OUTDIR}/mesa_per_step_contribution.csv", index=False)
print(f"-> saved {OUTDIR}/mesa_per_step_contribution.csv")
