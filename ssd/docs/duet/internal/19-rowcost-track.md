...
**B1 — NCCL env A/B: 무효 판정 후 중단 (2026-08-03 16:10~)**: base/
ring_ll128/ring_simple 3런이 균일하게 T_target ~150ms (정상 52.9) —
원인 규명: **타 사용자의 CPU 시뮬레이터 다중 실행으로 load avg 99 +
스왑 포화**. B=1 엔진은 커널 런치·NCCL 진행이 CPU 의존이라 target·
draft가 동일 배율로 감속 (GPU 클럭·PCIe는 정상 확인). 배치 중단,
**CPU 한가 시간대에 재실행 필요**. 유효성 재평가: 행당 2.36ms 실측
(짝런 13:0x, 오염 전)·E0/E1 결론 (오전~15:20) **모두 유효**; 8B TP1
판별(15:53)은 오염 경계라 재확인 딱지 (단, 차이-기반이라 정성 결론
유지 추정); nsys 구조 판정(NCCL이 diff의 전부)은 유지하되 정량은
불사용 원칙 그대로.

## 트랙 상태 (마감 정리 — 사용자 scope 지적 반영)

- **진단 완결**: 행당 비용 = NCCL 동기화 대기 (증거 사슬: 표준성분
  무혐의 → TP1 공짜 → TP2 span 오염 → nsys NCCL-지배).
- **남은 싼 수 1개**: NCCL env A/B (CPU 한가 시 ~1시간) — 이것만
  마치면 트랙 종료. 구조 개선(통신-계산 겹침 등)은 **몇 주짜리 —
  본 트랙 범위 밖**, 별도 캠페인으로만.
- 부산물: graph_pre +19% 미해결 이상의 원인 규명 (동일 기전).
