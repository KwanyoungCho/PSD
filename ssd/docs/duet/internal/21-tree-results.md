...
## 4.6 외부 리뷰 수용과 E1 상한 교정 (2026-08-04)

리뷰(사용자 전달) 전 항목 타당 판정 — 이슈 #21~#26으로 수정 완료
(20번 로그): KV staging(#21)·SHM 소비-대기(#22)·water-filling(#23)·
R-W 분리(#24)·트리 Policy-B DP(#25). 핵심 교정 = **E1 상한 재산정**
(#26, e1_feasibility/e1_explicit_tree.py):

| 모델 | per-hit L | 비고 |
|---|---|---|
| 관측 체인 | 1.794 | DP 자기검증 정확 재현 |
| 구 E1 [2,2,2,2] | 2.68~3.21 | **공유-트리 가정 — 명시 구현엔 30노드 필요 (무효)** |
| 명시 backbone+rescue b=6 | 2.033 (+13.3%) | |
| 명시 backbone+rescue b=8 | **2.137 (+19.1%)** | **control 실측 2.14와 정확 일치** — backbone이 상한 실현 중 |

**판정 함의**: per-hit +19% × P2-hit 비중(~0.25) ≈ **AL +2%대**가
교정된 토큰 이득 상한 (E1 헤드라인 +3.8%의 절반). 따라서 트리 순이득
은 **호스트 오버헤드(-15ms/step) 절감이 선결** — 토큰만으로는 시간
비용을 못 덮는다. 남은 지렛대: ① 오버헤드 절감(이슈 #20 롤백 후
저위험 재도전), ② P2-hit 비중 자체 상승(#25 DP의 seed 품질 효과 —
통합 프로브로 확인), ③ budget>8 형상(F_total 확장 — 시간과 트레이드).

**리뷰-수정 통합 검증 (#21~#25 적용, eslab18)**: R=6(#24) 구성이
신기록 — **P2 히트당 수락 2.19 (체인 +22%, #26 상한 2.137 도달/상회
— #25 DP의 seed 개선 효과 추정)**, P2 hit 0.249 유지 (17번 §3.2⑥
예측: top-집중의 hit 비용이 얇음), reject_all 0.12, dmax4 93%.
R=10 기본은 water-filling이 저-P_iv root까지 채워 얕은 서빙 혼입
...
**frontier 완결판 (2026-08-05, 새 rev fbaa9ce 전체 7런 — 18번
load 23-31 캐비앗, 리뷰3 미완 셀 해소)**: chain 앵커 74.78·P2AL
1.81 기준 — w6_nv4 59.3/1.87, w8_nv4 55.9/1.75, **w8_nv6
57.0/2.02**, w10_nv4 54.3/1.81, w10_nv6 52.1/1.96, w10_nv8
48.6/2.08. 결론: ① **토큰 이득은 Nv 단조 증가** (nv4 ≈0 — 이용률
60% 교란; nv6 +8~12%; nv8 +15%) — 구 "nv8만 이득" 반증. ② TPS는
W·Nv 모두에 단조 감소 (생성 노드 수 ∝ host 비용). ③ **w8_nv6 =
현행 스윗스팟** (w10_nv8 대비 TPS +8.4, 이득의 3/4) — T6 이후
챔피언 재선정 유력 후보. 원자료 frontier_fbaa9ce/.
...
| 평균 (min-max) | 체인 | 트리-R6 |
|---|---|---|
| Decode TPS | **80.49** (78.64-83.47) | 52.46 (49.87-54.55) |
| AL | 4.113 | 4.110 |
| P2 히트당 수락 | 1.827 | **2.060** (3/3 승) |
| P2 hit | 0.243 | 0.217 |

### 5-re. 수정-코드 재판정 (2026-08-04, eslab17 클린, 4-arm×3-cycle,
rev 8455b1e — 리뷰2 필수 대조군 chain-R6 포함, 사이클별 팔 순서 회전)

| 평균 (3-cycle) | chainR10 | chainR6 | treeR10 | **treeR6** |
|---|---|---|---|---|
| Decode TPS | 78.89 | 78.85 | 55.61 | 56.59 |
| P2 히트당 수락 | 1.81 | 1.79 | 1.93 | **2.12** |
| P2 hit | 0.250 | 0.226 | 0.247 | 0.219 |
| P1 AL | 3.98 | 4.00 | 4.00 | **4.06** |
| tok/step@hit | 4.32 | 4.37 | 4.37 | **4.53** |

AR 재실행 (크래시 수정 후): 33.48/33.35/33.28 → **33.37** (분산
0.1 — 클린박스 확증). async-SD C 78.20. 체인 = **2.36× vs AR**.

**귀속 (리뷰2가 요구한 분리)**: ① 선택편향(top-6 집중)만의 효과 =
chainR6−chainR10 = P2AL **−0.02 (≈0)** — 집중만으로는 깊이가 안
늘어남. ② topology 효과 = treeR6−chainR6 = P2AL **+0.33 (+18.4%)**
— 트리 이득은 **실제 topology 효과**로 확정. ③ hit 하락(−0.024)은
R6 집중의 성질로 체인에서도 동일 발생 — 트리 고유 손실은 −0.007
(≈0). ④ P1AL 4.06 ≥ 체인 4.00 — **P1 악화(#31) 해소 확인**.
⑤ 종합 토큰축: treeR6 tok/step@hit 4.53 = chainR10 **+4.9%** —
구-코드의 "AL 동률"이 수정으로 **토큰축 우위**로 전환.

**남은 문제는 시간축 단독**: TPS −28% (rollout host 12.4ms 등 —
§4.7 분해·처방 확정, T6 정적 템플릿로 −17~20ms/step 회수 예상).

**판정 (2차 리뷰 반영 정정)**: ① 히트당 수락 +12.8%는 실재·일관.
단 **chain-R6 대조군 없이는 topology 효과와 상위-root 선택편향이
미분리** (후속 대조군 필수). ② miss-이동 반영 회계로 **P2 축은 순
+0.048 tok/step 이득** — AL 동률의 진범은 **P1 축 −0.054** (트리-
스텝 P1 균등배분 #31 + backbone 예산소실 #27의 결과). "예산 보존
상쇄" 해석은 철회. ③ R6 예측 hit 0.220(top-6=90.7%) vs 실측 0.217
— "+0.6pt DP 이득" 주장 철회 (#29). ④ TPS −35% 중 트리-스텝
exit_logits 17.6ms는 #25 구현 자책 비용(#34). ⑤ 2.137은 상한이
아니라 한 형상의 예측값 — 전수 열거 최적은 2.2167 (#32). **종합:
현 결과는 "구조적 동률"이 아니라 "수정 가능한 구현 결함(#27·#28·
#31·#33·#34)이 이득을 지우는 상태" — 개선 여지 실재.**

## 5b. T4 sweep (구그리드 폐기 — 인터리브가 대체)
