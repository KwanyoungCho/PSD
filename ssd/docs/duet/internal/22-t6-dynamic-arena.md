...
## 1a-v3 실측 (2026-08-04, 숨은 동기화 제거 후 — 게이트① 근접)

| 지표 | chain | CPU tree | v2 | **v3** |
|---|---|---|---|---|
| P2 창 p50 | 12.35 | 21.49 | 25.95 | **20.27** ✓CPU 추월 |
| 간격 p50 | 0.27 | 9.21 | 13.58 | **8.36** ✓ |
| pre p50 | 2.14 | 7.40 | 11.41 | 11.36 ✗ (+4.0) |
| build→merge p50 | 14.70 | 36.66 | 47.45 | 40.87 (+4.2) |
| 스텝 평균 | 57.8 | 81.3 | 93.1 | **87.4** (+6.1) |

리뷰5의 1순위 용의자(boolean indexing nonzero→DtoH)가 실제 주범
— 제거만으로 창 −5.7·간격 −5.2ms. 잔여 +6.1ms의 소재: ① pre +4.0
(GPU 예산 계산 ~80런치 + f=0 준비 dispatch — CPU 파이썬 0.3ms보다
비쌈), ② post +1.4 (to_pool 3-sync + 잔여). 다음 레버 = 리뷰5 순서
그대로: **plan-ahead** (간격 바닥 8.36 안의 plan barrier ×4) +
...
**[정정 — 리뷰7]** 아래 "T6 트랙 완결"류 표현은 **GPU 장부 개선
사이클의 완결**로 한정한다. P2 시간 문제의 근본 해결이 아님 —
forward 간 ~6.6ms·to_pool→python view 포장 경로가 남아 있고, 후속
표준 계획은 **23번 (P2 연속 실행 구조)**. sweep·target 최적화는
23번 단계 완료 전 동결.
