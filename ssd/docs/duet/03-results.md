...
이 문서는 DUET-SSD 의 모든 실험 결과를 시간순 + 모델 크기 순으로 통합한다.
원본은 `DUET-RESULTS.md` (v1 초기 측정), `DUET-rev1-RESULTS.md` (Rev1
Policy A), `DUET-SWEEP-RESULTS.md` (parameter sweep, clean-GPU rerun),
`DUET-PHASE2-HYBRID-{REPORT,FINAL-REPORT}.md` (Phase 2 hybrid 결과) 다섯
파일이다.

34B / 70B 의 양자화 + DUET 최종 결과는 `quantization/03-final-report.md`
...
- DUET: max(65, 54.5) = 65 ms → **여전히 target bound**
...
2. **`_build_tree_batch` 리팩토링** — DUET 모드에서 full_layout
...
4. **1-pass DUET** (alternative) — full_layout 단일 decode 유지하되, fork
...
- `assert jit_speculate` (DUET 사용 시 강제)
- `bench.py` 에서 `--duet` → `jit_speculate=True` 자동 설정
...
| 메트릭 | Baseline SSD | DUET v1 (고정) | DUET Rev1 (Policy A) |
|--------|-------------|----------------|---------------------|
...
| 메트릭 | Baseline SSD | DUET Rev1 (Policy A) |
|--------|-------------|---------------------|
...
- `_select_proxy_sourced_tokens` 에서 `duet_proxy["accept_probs"].shape`
...
- **Target**: LayerSkip-Llama3-8B (32 layers) [DUET / SSD baselines]
- **Target (EAGLE track)**: Llama-3.1-8B-Instruct
- **Draft (SSD/DUET)**: Llama-3.2-1B-Instruct
...
- **Speculation knobs**: `--async --spec --k 4`, baselines vary
  `--f ∈ {2,3,4,5}` to match DUET's total draft budget (`MQ_LEN = f·(K+1)`)
...
### Phase A — DUET (exit_layer=21 fixed, sweep f × draft_fan_out split)
...
|--------|---|------------------|------------|--------|----------|----------|------------|-------------|
| duet_f2_dfo1 | 2 | (1, 1) | 88.58 | 0.83 | 0.87 | 4.31 | 43.71 | 24.79 |
| **duet_f3_dfo1** | **3** | **(1, 2)** | **87.54** | 0.81 | 0.88 | 4.24 | 43.40 | 24.75 |
| duet_f3_dfo2 | 3 | (2, 1) | 85.29 | 0.79 | 0.87 | 4.15 | 43.74 | 24.64 |
| duet_f4_dfo1 | 4 | (1, 3) | 85.29 | 0.79 | 0.88 | 4.18 | 43.93 | 24.72 |
| duet_f4_dfo2 | 4 | (2, 2) | 85.47 | 0.79 | 0.88 | 4.18 | 43.84 | 24.77 |
| duet_f4_dfo3 | 4 | (3, 1) | 87.10 | 0.82 | 0.88 | 4.26 | 44.02 | 24.65 |
| duet_f5_dfo1 | 5 | (1, 4) | 82.91 | 0.80 | **0.90** | 4.21 | 45.17 | 24.89 |
| duet_f5_dfo2 | 5 | (2, 3) | 86.76 | 0.81 | 0.89 | 4.26 | 43.99 | 24.73 |
| duet_f5_dfo3 | 5 | (3, 2) | 86.37 | 0.81 | 0.89 | 4.24 | 44.11 | 24.75 |
| duet_f5_dfo4 | 5 | (4, 1) | 82.19 | 0.80 | 0.88 | 4.18 | 45.93 | 25.58 |

모든 DUET 구성이 좁은 band (82–89 tok/s) 에 모임. **모든 DUET 구성이
baseline_f3 보다 35–42% 느림**, *심지어* 모든 DUET 구성이 더 높은
...
|--------|------------------------|------------|--------|----------|----------|------------|-------------|
| duet_f3_dfo1_exit10 | 10 (31%) | 85.00 | 0.79 | 0.85 | 4.15 | 43.83 | 24.75 |
| **duet_f3_dfo1_exit16** | **16 (50%)** | **88.41** | **0.81** | 0.87 | **4.26** | 43.18 | 24.71 |
| duet_f3_dfo1 (=21) | 21 (66%) | 87.54 | 0.81 | 0.88 | 4.24 | 43.40 | 24.75 |
| duet_f3_dfo1_exit26 | 26 (81%) | 86.54 | 0.80 | 0.89 | 4.21 | 43.64 | 24.72 |
...
에서 Baseline SSD 가 AR 대비 ~1.9× (74 → 142), DUET 는 ~1.2× (74 → 87).
**DUET 는 여전히 AR 보다 빠르지만, baseline SSD 를 따라잡지 못함**.
...
| DUET SSD | LayerSkip-Llama3-8B | f=3, dfo=1, exit=16 | 88.41 | 1.19× | **0.62×** |
| EAGLE | Llama-3.1-8B | — | *OOM (not run)* | — | — |

### Root-Cause Analysis

1. **Token efficiency 는 실재하지만 작다** — DUET 가 cache hit 을 +0.02–
...
2. **Draft step 이 bottleneck, 그리고 baseline 의 ~2 배** — Baseline 은
   24–26 ms, DUET 는 모든 config 에서 43–46 ms. 차이는 2-pass 구조가 지배:
...
3. **Target verify 가 split CudaGraph 로 더 빨라지지 않음** — Baseline
   23 ms, DUET (graph_pre + proxy + graph_post) 24.7–25.6 ms. CudaGraph
...
- `duet_f2_dfo1` 와 `duet_f3_dfo2` 가 한 번 `DistStoreError` 로 실패했음
  (rapid slot reuse 시 multiprocessing-spawn race on port reuse). 같은
  hardware 에서 rerun 하여 위 표의 수치를 얻음. 알고리즘적 실패 아님.
- `bench/run_duet_sweep.sh` 의 summary regex 도 "Total Throughput:" 를
...
- **CodeLlama-34B** + Llama-3.2-1B draft (TP=4 target, TP=1 draft) — DUET
  이득이 실제로 발현되는 범위. 결과는
  `quantization/03-final-report.md` (target AWQ + draft AWQ 적용 시
  DUET 의 wall-time vs token-efficiency tradeoff 가 어떻게 바뀌는지) 에 정리.
- **layerskip-llama2-70B** + TinyLlama draft (TP=4 target, TP=1 draft) —
  AWQ 양자화 후 70B 가 single-node 에 적재되며 DUET 검증. `proxy_compute_send`
...
`DUET-PHASE2-HYBRID-REPORT.md` + `DUET-PHASE2-HYBRID-FINAL-REPORT.md` 의
...
- **Profiling**: `SSD_PROFILE_DUET=1` 로 zero-sync CUDA event 기반 per-phase
  ms/step 측정
...
build 전체 비용 3.36 → 3.51 ms (+0.15 ms = sub-label 8 회 duet_record/close
...
```
[DUET] Captured draft glue_short CG (K_short+1=N)
[DUET hybrid] Captured 2-bucket verify CG ... long(K=...) + short(K=...)
...
[DUET hybrid] capturing phase2_hybrid CG bucket=long total_rows=...
[DUET hybrid] capturing phase2_hybrid CG bucket=short total_rows=...
```
