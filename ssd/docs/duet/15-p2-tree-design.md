...
사용자 판단** [사용자 결정 2026-08-04]). **상태: T1 구현 착수 (2026-08-03 사용자 승인 — G0 관문 완료 판정:
E0/E1/E2 결과 보고 후 "tree 구현 진행, 행당-비용 A/B는 구현 이후로
연기" 결정. 현 비용 기준 기대 +0.3~0.7%, 행당 비용 개선 결합 시
+2.1~2.6% — 최종 판정은 G1 실측).**
...
### 7.5 hit-step 후속 파이프라인 (트리 응답의 "다음 step" 연쇄 — 결정 ④로 확정)
...
**확정 (결정 ④, 사용자 2026-08-04) — 기존 규칙의 전 노드 일반화**:
3계 모두 트리 노드 전체에 현행 기계를 일반화한다. backbone(최상위
체인) 축소안은 **폐기** — 수락 보행이 backbone을 이탈하는 사건은
...
- ⓒ **Policy B 트리 일반화**: 노드별 α̂ = min(1, p^E_n(tok_n)/
  ...
  **응답에 이미 포함**되어 추가 데이터 없음. ĥ는 "보행이 노드 n에서
  끝날 확률" = 경로-곱 × 앞형제 (1−α̂) 곱으로 일반화 — **priority와
