...
**판정**: ① 히트당 수락 +12.8%는 실재·일관 (교정 상한 실현) —
그러나 R6 무효화의 hit 희생(−13%, 설계값)과 **정확히 상쇄**되어
AL 동률. 예산 제약(K2·W) 안에서 깊이↔히트 프런티어가 평평함이
실증됨. ② TPS는 시간 오버헤드로 체인 band-clear (−35%; 슬림화
−4.2ms/step 반영 전 코드). ③ 트리 승리의 잔여 경로: 예산 확장
(F_total>K2 — 시간 비용과 교환), seed 품질(hit 자체 상승 — #25
DP 소폭 효과 관측: 실측 0.217 > 순수무효화 예측 0.211), 시간축
잔여 회수. **판단은 사용자 몫** (판정 철학).
