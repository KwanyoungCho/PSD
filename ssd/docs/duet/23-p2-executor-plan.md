...
## P2AL 1.27 조사 (2026-08-06, 리뷰 지시 수행)

### 커밋 이분 (17번 동일서버·동일seed42·numseqs15/384, 1런씩)
| REV | 활성 변경 | P2AL | hit | P2hit |
|---|---|---|---|---|
| 2ed7e2c | (v1 기준) | 2.04 | 0.75 | 0.203 |
| 41cc532 | +self-bit 수정 | 1.95 | 0.77 | 0.219 |
| c9e81a3 | +dbg버퍼/게이트v1 (45% arena fallback) | 1.97 | 0.80 | 0.241 |
| HEAD sub=1 | +canvas 대체 | **1.84** | 0.75 | 0.226 |
| HEAD sub=0 | canvas -1 유지 | **1.90** | 0.78 | 0.234 |
| (참고) a2ecfb4 런 | 동일 의미 코드 | 1.27 | 0.80 | 0.236 |

- **self-bit 무죄, dbg 무죄** (각 1.95/1.97)
- **canvas 대체 무죄 방향**: isolated dup-page 3-way 9/9 bit-동일
  (page0중복==마지막중복==전용sentinel, tests/diag/test_canvas_dup
  _page.py) + HEAD 동일의미 재런 1.84
- **1.27/1.45는 같은 서버·같은 seed·같은 의미 코드에서 재현 안 됨**
  → 단일런 이상치 개연 — P2AL의 런간 변동이 기존 밴드(±0.2)보다
  클 수 있음이 드러남 (numseqs 15에서 P2-hit 표본 ~1.3k, seq 구성
  민감). **표본 없는 단일런 P2AL로 arm 판정 금지** 교훈.

### 진행: 3-arm × 3-seed 순서회전 (17·18 병렬, STAGE2 원시카운트)
arena / exec(sub=1) / exec(sub=0 -1유지) — P2AL 분산과 arm 효과를
표본으로 확정. 이 결과로 canvas 최종 방식(대체 vs 유지 vs 전용
더미 vs fallback)을 증거로 선택 (좋은 값 고르기 금지 — 동일성
확인이 목적).

## P2AL 원인 확정 (2026-08-06) — canvas -1 = 간헐 NaN 오염 (실증)

### 메커니즘 (tests/diag/test_canvas_nan_poison.py로 고정)
mask=0이어도 canvas 페이지에 **Inf/NaN이 있으면 fa2 출력 전체가
NaN** (산술식 마스킹의 0×inf). 유한값이면 bit-동일 무해 (중복
페이지 포함 — test_canvas_dup_page 9/9). 따라서 페이지 **-1(텐서
밖 OOB 읽기)은 메모리 내용에 따라 간헐적으로 해당 스텝의 P2 트리
전체를 오염** — v1 이후 모든 exec 측정에 이 간헐 오염이 혼입돼
있었다 (hit −4.6%p·P2AL 심저 런들의 유력 원천).

### tri-AB 확증 (3-arm × 3-seed × 2박스 = 18런, 순서회전,
raw 분모 기반, 페어드 6쌍)
| arm | P2AL (6런) | μ | hit(raw) μ | paired ΔP2AL | paired Δhit |
|---|---|---|---|---|---|
| arena | 1.81–1.97 | 1.913 | 0.8128 | — | — |
| e1(유효 대체) | 1.56–1.99 | 1.848 | 0.7830 | **−0.065** | **−0.0298** |
| e0(-1 유지) | **1.04**–1.89 | 1.588 | 0.7821 | **−0.325** | −0.0307 |

- **e0 폐기 확정**: 6/6 음수, 심저 1.04(클린박스 포함) — NaN 간헐
  오염과 정합. arena는 6/6 안정(1.81+).
- **e1 규약 확정** (토글 제거, 유일 규약): canvas 슬롯은 반드시
  유한값 보장 유효 페이지. 리뷰 지시의 '전용 dummy'와 동등 효과
  (중복-페이지 무해 실증) — 블록 소유권상 dbt[0] 재사용.
- 이전 "P2AL 1.27/1.45" 단일런들은 이 간헐 오염의 발현 — 커밋
  이분에서 동일 코드 재런이 1.84-1.90으로 재현 실패했던 이유.

### 남은 갭 (e1 기준, 기준 §6 대조)
- ΔP2AL −0.065: 기준(−0.05) 미세 초과 — 6쌍 중 1쌍(−0.25)이 견인,
  나머지 5쌍 μ −0.028. 표본 확대 시 재평가.
- Δhit −2.98%p: 기준(−1%p) 초과 — taxonomy 페어드에서
