...
| P1 prior에 앞형제-기각 인자 누락 | 확정(모델) → **원복(판정불가)** | 수학 지적은 맞음 (둘째 형제 2.08× 과대). 단 라이브 이득/회귀는 **로컬 단일 스모크로 판정 불가** — 같은 코드 반복런에서 P1AL 4.06↔3.71, P2AL 1.94↔2.12 (8×256 노이즈 밴드 ±0.2). 챔피언-동작(재판정 8455b1e과 수학 동일)으로 원복 유지 — 입증 부담은 변경 쪽 (재도전 = eslab17 인터리브 A/B 필수) |
| Policy-B가 temp/sampler_x 미적용 | 확정(모델) → **원복(판정불가)** | 동상 — 미러판 1.88/1.94 vs 기준 2.13은 노이즈 밴드 내. plain softmax(체인 일관·재판정 검증 동작점) 유지; 재도전은 P_iv 랭킹·β·prior 공동 recalibration + 인터리브 게이트. temp0 명시 게이트는 유지 |
...
green). #36·#37은 원복 — 1차 판정 "실측 회귀"는 반복런으로 **번복**
(같은 코드에서 P2AL 1.94↔2.12: 로컬 8×256 단일런의 AL 분해능
±0.2). 확정 교훈 2개: ① 라이브 경로 변경의 채택 게이트는 로컬
스모크가 아니라 **클린박스 인터리브 A/B** (AL ±0.05 분해능); ②
스모크의 역할은 무크래시·불변량 검증까지. #36·#37 재도전은 T6 후
인터리브 arm으로.
